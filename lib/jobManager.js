...
jobManager.prototype.restartJobs = function () {
    ...
    Job.find({ active: true }).exec().then(function (data) {
        ...
        jobCursor.forEach(function (job) {
            if (!job)
                return;
            self.addJob(job.url, '1 minute', job.actions.split(','), job.title, job.user);
        })
    });
};

//url,interval,actions,title
jobManager.prototype.addJob = function (url, interval, actions, title, userId) {
    var jobname = this.defineJob(url, actions, userId);
...
jobManager.prototype.defineJob = function (url, actionConfig, userId) {
